export { Overlay } from '@minimalist-ui/core/overlay/Overlay';
export type { OverlayInstance, OverlayProps } from '@minimalist-ui/core/overlay/types';
export { useInert } from '@minimalist-ui/core/overlay/useInert';
export { usePosition } from '@minimalist-ui/core/overlay/usePosition';
export { overlayManager } from '@minimalist-ui/core/overlay/overlayManager';
export {
    isOverlayProviderMounted,
    markOverlayProviderMounted,
    markOverlayProviderUnmounted
} from '@minimalist-ui/core/overlay/overlayContext';
export { ensureBackdrop } from '@minimalist-ui/core/overlay/ensureBackdrop';
export { Backdrop } from '@minimalist-ui/core/overlay/Backdrop';
export { OverlayProvider } from '@minimalist-ui/core/overlay/OverlayProvider';
